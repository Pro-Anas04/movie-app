                ...
                bannerData.map((data,index) => {
                    return(
                        <div key={data.id+"bannerHome"+index} className='min-w-full min-h-[450px] lg:min-h-full overflow-hidden relative group transition-all' style={{transform: `translateX(-${currentImage * 100}%)`}}>
                            ...
                            {/* bottom next and previous image */}
                            ...
                            <div className='container mx-auto'>
                                <div className=' w-full absolute bottom-0 max-w-md px-3'>
                                    ...
                                    <div className='flex items-center gap-4'>
                                        <p className='flex justify-center items-center gap-1'><FaStar className='text-amber-400'/>Rating : {Number(data.vote_average).toFixed(1)}+</p>
                                        <span>|</span>
                                        <p>View : {Number(data.popularity).toFixed(0)}</p>
                                    </div>
                                    <Link to={"/"+data?.media_type+"/"+data.id}>
                                        <button className='bg-white px-4 py-2 text-black font-bold rounded mt-4 cursor-pointer hover:bg-gradient-to-l from-red-500 to-amber-500 shadow-md transition-all hover:scale-105'>
                                            Play Now
                                        </button>
                                    </Link>
                                </div>
